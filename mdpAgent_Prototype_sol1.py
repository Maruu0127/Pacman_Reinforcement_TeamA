...
#
# Attribution Information: The Pacman AI projects were developed at UC Berkeley.ls
...
class MDPAgent(Agent):
	...
	def makeValueMap(self, state):
		...
		def getRangeDistance(state):
			# Calculate Euclidean distance between ghost and pacman positions
			
			state1 = api.whereAmI(state)
			state2 = api.ghosts(state)
		...
		for i in valueMap.keys():
			for j in range(len(ghosts)):
				...
				distance = getRangeDistance(i, ghostPosition)

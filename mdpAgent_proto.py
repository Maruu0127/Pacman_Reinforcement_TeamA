...
class MDPAgent(Agent):
	...
	def makeValueMap(self, state):
		...
		# Update function for ghost's location

		for i in valueMap.keys():
			for j in range(len(ghosts)):
				ghostPosition = (int(ghosts[j][0]), int(ghosts[j][1]))
				ghostTime = ghostStates[j][1]
				distance = getDistance(i, ghostPosition)
				
				if distance <= 3:
					# If the distance is close, assign a highly negative utility
					valueMap[i] = -100
				elif distance <= 8:
					# If the distance is moderately close, assign a negative utility
					valueMap[i] = -50
				elif ghostTime >= 5:
					# If the ghost is weak, assign a positive utility
					valueMap[i] = 10
				else:
					# Otherwise, assign a neutral utility
					valueMap[i] = 0
		return valueMap
	...
	def getAction(self, state):
		...
		if maxWidth >= 10 and maxHeight >= 10:
			...
			self.valueIterationSmall(state, 0.1, 0.7, valueMap)
